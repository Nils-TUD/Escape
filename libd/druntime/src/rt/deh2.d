...
//int count = 0;
    while (1)           // for each function on the stack
    {
        ...
        {   // if end of call chain
            debug(LOGDEH) printf("end of call chain\n");
            break;
        }

        debug(LOGDEH) printf("found caller, EBP = x%x, retaddr = x%x\n", regebp, retaddr);
//if (++count == 12) *(char*)0=0;
        ...
        {
            debug(LOGDEH) printf("no handler table\n");
            continue;
        }
