...
else version(Escape)
{
	private
    {
        ...
        extern (C) void* thread_entryPoint( void* arg )
        {
            ...
            assert( obj.m_curr == &obj.m_main );
            Thread.add( &obj.m_main );
            bool res = setThreadVal(obj.sm_this,cast(void*)obj);
            assert(res);
...
class Thread
{
    ...
    static void sleep( /*Interval*/ ulong interval )
    {
        ...
        {
        	if(.sleep(cast(uint)(interval / Interval.milli)) < 0)
...
extern (C) void thread_init()
{
    ...
    else version( Escape )
    {
        ...
        mainThread.m_addr    = gettid();
        mainContext.bstack = getStackBottom();
        mainContext.tstack = mainContext.bstack;

        mainThread.m_isRunning = true;
        
        bool res = setThreadVal(Thread.sm_this, cast(void*)mainThread);
        assert(res);
    }
