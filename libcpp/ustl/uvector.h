...
public:
	...
	inline operator cmemlink(void) const {
		return (cmemlink(m_Data));
	}
	inline operator cmemlink(void) {
		return (cmemlink(m_Data));
	}
	inline operator memlink(void) {
		return (memlink(m_Data));
	}
	...
	inline size_type max_size(void) const {
		return (m_Data.max_size() / sizeof(T));
	}
	...
	inline void manage(pointer p, size_type n) {
		m_Data.manage(p, n * sizeof(T));
	}
	...
	inline void link(const_pointer p, size_type n) {
		m_Data.link(p, n * sizeof(T));
	}
	inline void link(pointer p, size_type n) {
		m_Data.link(p, n * sizeof(T));
	}
	inline void link(const vector<T>& v) {
		m_Data.link(v);
	}
	inline void link(vector<T>& v) {
		m_Data.link(v);
	}
	inline void link(const_pointer first, const_pointer last) {
		m_Data.link(first, last);
	}
	inline void link(pointer first, pointer last) {
		m_Data.link(first, last);
	}
	inline void read(istream& is) {
		container_read(is, *this);
	}
	inline void write(ostream& os) const {
		container_write(os, *this);
	}
	inline void text_write(ostringstream& os) const {
		container_text_write(os, *this);
	}
	inline size_t stream_size(void) const {
		return (container_stream_size(*this));
	}
protected:
...
inline void vector<T>::reserve(size_type n, bool bExact) {
	const size_type oldCapacity = m_Data.capacity() - m_Data.capacity()
			% sizeof(T);
	m_Data.reserve(n * sizeof(T), bExact);
	construct(iterator(m_Data.begin() + oldCapacity), iterator(m_Data.begin()
			+ m_Data.capacity()));
}
...
template<typename T>
inline typename vector<T>::iterator vector<T>::insert_space(iterator ip,
		size_type n) {
...
template<typename T>
inline typename vector<T>::iterator vector<T>::insert(iterator ip, size_type n,
		const T& v) {
...
template<typename T>
inline typename vector<T>::iterator vector<T>::insert(iterator ip,
