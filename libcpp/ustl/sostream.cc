...
ostringstream::ostringstream(void* p, size_t n) :
	ostream(), m_Buffer(), m_Flags(0), m_Width(0), m_Base(10), m_Precision(2) {
	exceptions(goodbit);
	link(p, n);
}
...
ostringstream::ostringstream(const string& v) :
	ostream(), m_Buffer(v), m_Flags(0), m_Width(0), m_Base(10), m_Precision(2) {
	exceptions(goodbit);
...
ostringstream& ostringstream::write(const void* buffer, size_type sz) {
	...
	for (size_type bw = 0; (bw = min(sz, remaining() ? remaining() : overflow(
			sz))); buf += bw, sz -= bw)
