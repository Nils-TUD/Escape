...
std::__exception_ptr::exception_ptr::exception_ptr(const exception_ptr& other) throw () :
...
std::__exception_ptr::exception_ptr::operator=(const exception_ptr& other) throw () {
	exception_ptr(other).swap(*this);
	return *this;
}

void std::__exception_ptr::exception_ptr::_M_addref() throw () {
	if(_M_exception_object) {
		__cxa_refcounted_exception *eh = __get_refcounted_exception_header_from_obj(
				_M_exception_object);
		__sync_add_and_fetch(&eh->referenceCount,1);
	}
}

void std::__exception_ptr::exception_ptr::_M_release() throw () {
	if(_M_exception_object) {
		__cxa_refcounted_exception *eh = __get_refcounted_exception_header_from_obj(
				_M_exception_object);
...
void* std::__exception_ptr::exception_ptr::_M_get() const throw () {
	return _M_exception_object;
}

void std::__exception_ptr::exception_ptr::_M_safe_bool_dummy() {
}

void std::__exception_ptr::exception_ptr::swap(exception_ptr &other) throw () {
...
bool std::__exception_ptr::exception_ptr::operator!() const throw () {
...
bool std::__exception_ptr::operator==(const exception_ptr& lhs,const exception_ptr& rhs) throw () {
...
std::exception_ptr std::current_exception() throw () {
	...
	return std::exception_ptr(__get_object_from_ambiguous_exception(header));
}

static void __gxx_dependent_exception_cleanup(_Unwind_Reason_Code code,_Unwind_Exception *exc) {
	// This cleanup is set only for dependents.
	__cxa_dependent_exception *dep = __get_dependent_exception_from_ue(exc);
	__cxa_refcounted_exception *header = __get_refcounted_exception_header_from_obj(
...
void std::rethrow_exception(std::exception_ptr ep) {
	void *obj = ep._M_get();
	__cxa_refcounted_exception *eh = __get_refcounted_exception_header_from_obj(obj);
