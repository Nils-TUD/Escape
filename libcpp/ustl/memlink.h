...
public:
	...
	inline memlink(void* p, size_type n) :
		cmemlink(p, n) {
	}
	inline memlink(const void* p, size_type n) :
		cmemlink(p, n) {
	}
	inline memlink(rcself_t l) :
		cmemlink(l) {
	}
	...
	inline const_iterator iat(size_type i) const {
		return (cmemlink::iat(i));
	}
	size_type writable_size(void) const {
		return (size());
	}
	inline rcself_t operator=(const cmemlink& l) {
		cmemlink::operator=(l);
		return (*this);
	}
	...
	void fill(iterator start, const void* p, size_type elsize,
			size_type elCount = 1);
