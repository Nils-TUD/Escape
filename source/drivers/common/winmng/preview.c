...
static void preview_handleIntersec(uint8_t *shmem,sRectangle *curRec,sRectangle *intersec,size_t i,
		gsize_t xres,gsize_t yres);
static void preview_clearRegion(uint8_t *shmem,gpos_t x,gpos_t y,gsize_t w,gsize_t h);
static void preview_copyRegion(uint8_t *src,uint8_t *dst,gsize_t width,gsize_t height,gpos_t x1,gpos_t y1,
...
void preview_set(uint8_t *shmem,gpos_t x,gpos_t y,gsize_t width,gsize_t height,gsize_t thickness) {
	...
	if(thickness > 0) {
		if(thickness != previewThickness || width != previewRect.width ||
				height != previewRect.height) {
			previewRectCopies[0] = (uint8_t*)malloc(width * thickness * pxSize);
			previewRectCopies[1] = (uint8_t*)malloc(height * thickness * pxSize);
			previewRectCopies[2] = (uint8_t*)malloc(width * thickness * pxSize);
			previewRectCopies[3] = (uint8_t*)malloc(height * thickness * pxSize);
		}
