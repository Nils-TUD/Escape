...
extern (C){
	...
	else version (Escape)
	{
		//const int BUFSIZ      = 0x4000;
		const int EOF           = -1;
		const int FOPEN_MAX     = 64;
		const int FILENAME_MAX  = 50;
		const int TMP_MAX       = 0; // TODO ??
		const int L_tmpnam      = 0; // TODO ??
	}
    else version( linux )
    {
        //const int BUFSIZ      = 0x4000;
...
extern(C){
    //TODO int remove(in char* filename);
    //TODO int rename(in char* from, in char* to);

    //TODO FILE* tmpfile();
    //TODO char* tmpnam(char* s);

    int   fclose(FILE* stream);
    int   fflush(FILE* stream);
    FILE* fopen(in char* filename, in char* mode);
    //TODO FILE* freopen(in char* filename, in char* mode, FILE* stream);

    //TODO void setbuf(FILE* stream, char* buf);
    //TODO int  setvbuf(FILE* stream, char* buf, int mode, size_t size);
...
extern(C) {
    ...
    //TODO int fgetpos(FILE* stream, fpos_t * pos);
    //TODO int fsetpos(FILE* stream, in fpos_t* pos);
