...
debug(MODINIT) private import tango.stdc.stdio;
...
{
    debug(MODINIT) printf("_moduleCtor()\n");
    version (linux)
    {
        int len = 0;
        ModuleReference *mr;
    ...
    debug(MODINIT) printf("_moduleinfo_dtors = x%x\n", cast(void *)_moduleinfo_dtors);
    _moduleCtor2(_moduleinfo_array, 0);
}

void _moduleCtor2(ModuleInfo[] mi, int skip)
{
    debug(MODINIT) printf("_moduleCtor2(): %d modules\n", mi.length);
    for (uint i = 0; i < mi.length; i++)
    {
        ModuleInfo m = mi[i];

        debug(MODINIT) printf("\tmodule[%d] = '%p'\n", i, m);
        if (!m)
            continue;
        debug(MODINIT) printf("\tmodule[%d] = '%s'\n", i, m.name.ptr);
        if (m.flags & MIctordone)
            continue;
        debug(MODINIT) printf("\tmodule[%d] = '%s', m = x%x\n", i, m.name.ptr, m);
...
{
    debug(MODINIT) printf("_moduleDtor(): %d modules\n", _moduleinfo_dtors_i);
    for (uint i = _moduleinfo_dtors_i; i-- != 0;)
    {
        ModuleInfo m = _moduleinfo_dtors[i];

        debug(MODINIT) printf("\tmodule[%d] = '%s', x%x\n", i, m.name.ptr, m);
        if (m.dtor)
        {
            (*m.dtor)();
        }
    }
    debug(MODINIT) printf("_moduleDtor() done\n");
}
...
{
    debug(MODINIT) printf("_moduleUnitTests()\n");
