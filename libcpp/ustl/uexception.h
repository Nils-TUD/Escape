...
public:
	...
	inline virtual size_t stream_size(void) const {
		return (sizeof(m_Format) + sizeof(uint32_t) + m_Backtrace.stream_size());
	}
	...
	inline xfmt_t format(void) const {
		return (m_Format);
	}
	inline rcbktrace_t backtrace(void) const {
		return (m_Backtrace);
	}
protected:
	inline void set_format(xfmt_t fmt) {
		m_Format = fmt;
	}
private:
...
public:
	inline explicit bad_cast(void) throw () :
		exception() {
	}
	inline virtual const char* what(void) const throw () {
		return ("bad cast");
...
public:
	stream_bounds_exception(const char* operation, const char* type,
			uoff_t offset, size_t expected, size_t remaining) throw ();
	inline virtual const char* what(void) const throw () {
