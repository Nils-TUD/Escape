...
public:
	...
	inline tuple(void);
	explicit inline tuple(const_reference v0, const_reference v1 = T(),
			const_reference v2 = T(), const_reference v3 = T());
	...
	inline size_t stream_size(void) const {
		return (nr_container_stream_size(*this));
	}
private:
	T m_v[N];
};
...
inline tuple<N, T>::tuple(const_reference v0, const_reference v1,
		const_reference v2, const_reference v3) {
	m_v[0] = v0;
	if (N > 1)
		m_v[1] = v1;
	if (N > 2)
...
template<size_t N, typename T1, typename T2>
inline const tuple<N, T1> operator+(const tuple<N, T1>& t1,
		const tuple<N, T2>& t2) {
	tuple<N, T1> result;
	for (uoff_t i = 0; i < N; ++i)
