        ...
        else version (Escape)
        {
            ...
            if (_pid >= 0)
            {
                ...
                else
                {
                    ...
                    // Replace stdout with the "write" pipe
                    redirFd(STDOUT_FILENO,pout.source.fileHandle());
                    //pout.source.close();
                    //scope(exit)
                    //    pout.sink.close();

                    // Replace stderr with the "write" pipe
                    redirFd(STDERR_FILENO,perr.source.fileHandle());
                    //perr.source.close();
                    //scope(exit)
                    //    perr.sink.close();
                    ...
                    // Convert the arguments variables to
                    // the format expected by the execv() family of functions.
                    ...
                    if (_workDir.length > 0)
                    {
                    	setEnv("CWD",_workDir.ptr);
                    }

                    // Replace the child fork with a new process.
                    rc = exec(_args[0], argptr);
                    if (rc == -1)
                    {
                        Stdout.formatln("Failed to exec {0}: {1}",
                                        _args[0], SysError.lastMsg);
