...
public:
	...
	inline explicit auto_ptr(pointer p = NULL) :
		m_p(p) {
	}
	/// Takes ownership of pointer in \p p. \p p relinquishes ownership.
	inline auto_ptr(auto_ptr<T>& p) :
		m_p(p.release()) {
	}
	/// Deletes the owned pointer.
	inline ~auto_ptr(void) {
		delete m_p;
	}
	...
	/// Takes ownership of \p p.
	inline auto_ptr<T>& operator=(pointer p) {
		reset(p);
		return (*this);
	}
	/// Takes ownership of pointer in \p p. \p p relinquishes ownership.
	inline auto_ptr<T>& operator=(auto_ptr<T>& p) {
		reset(p.release());
		return (*this);
	}
	...
	inline bool operator==(const auto_ptr<T>& p) const {
		return (m_p == p.m_p);
	}
	inline bool operator<(const auto_ptr<T>& p) const {
		return (p.m_p < m_p);
...
struct Sdtorsr {
	inline void operator()(T first, T last) throw () {
		dtors(first, last);
	}
};
template<typename T>
...
inline void destroy(ForwardIterator first, ForwardIterator last) throw () {
	typedef typename iterator_traits<ForwardIterator>::value_type value_type;
	Sdtorsr<ForwardIterator, numeric_limits<value_type>::is_integral> ()(first,
			last);
}

/// Casts \p p to the type of the second pointer argument.
template<typename T> inline T* cast_to_type(void* p, const T*) {
	return ((T*) p);
}
...
///
template<typename T>
inline pair<T*, ptrdiff_t> make_temporary_buffer(void* p, size_t n,
		const T* ptype) {
...
///
template<typename InputIterator, typename ForwardIterator>
ForwardIterator uninitialized_copy_n(InputIterator first, size_t n,
		ForwardIterator result) {
...
///
template<typename ForwardIterator, typename T>
ForwardIterator uninitialized_fill_n(ForwardIterator first, size_t n,
		const T& v) {
	for (++n; --n; ++first)
		construct(&*first, v);
	return (first);
