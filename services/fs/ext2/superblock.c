 ...
 */
static tInodeNo ext2_allocInodeIn(sExt2 *e,u32 groupStart,sBlockGroup *group);
/**
...
tInodeNo ext2_allocInode(sExt2 *e,sCachedInode *dirInode) {
	...
	ino = ext2_allocInodeIn(e,group * e->superBlock.inodesPerGroup,e->groups + group);
	if(ino != 0)
		return ino;

	/* now try the other block-groups */
	for(i = group + 1; i != group; i = (i + 1) % gcount) {
		ino = ext2_allocInodeIn(e,i * e->superBlock.inodesPerGroup,e->groups + i);
		if(ino != 0)
			return ino;
	}
	return 0;
