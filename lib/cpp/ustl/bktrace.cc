...
#include "sostream.h"
#include "mistream.h"
/* TODO we don't have these functions yet */
#if 0 && linux && __GNUC__
#include <execinfo.h>
#else
...
CBacktrace::CBacktrace(void) throw () :
	m_Symbols(NULL), m_nFrames(0), m_SymbolsSize(0) {
...
size_t CBacktrace::stream_size(void) const {
	return (Align(stream_size_of(m_nFrames) + stream_size_of(m_SymbolsSize)
			+ m_nFrames * sizeof(void*) + m_SymbolsSize));
}
