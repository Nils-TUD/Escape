...
public:
	...
	inline bool operator==(const reverse_iterator& iter) const {
		return (m_i == iter.m_i);
	}
	inline bool operator<(const reverse_iterator& iter) const {
		return (iter.m_i < m_i);
	...
	inline reverse_iterator& operator+=(size_t n) {
		m_i -= n;
		return (*this);
	}
	inline reverse_iterator& operator-=(size_t n) {
	...
	inline reverse_iterator operator+(size_t n) const {
		return (reverse_iterator(m_i - n));
	}
	inline reverse_iterator operator-(size_t n) const {
		return (reverse_iterator(m_i + n));
	}
	inline reference operator[](uoff_t n) const {
		return (*(*this + n));
	}
	inline difference_type operator-(const reverse_iterator& i) const {
		return (distance(m_i, i.m_i));
	}
protected:
	Iterator m_i;
};
...
public:
	explicit insert_iterator(Container& ctr, iterator ip) :
		m_rCtr(ctr), m_ip(ip) {
	}
	...
	inline insert_iterator& operator*(void) {
		return (*this);
	}
	inline insert_iterator& operator++(void) {
		++m_ip;
...
template<class Container>
inline insert_iterator<Container> inserter(Container& ctr,
		typename Container::iterator ip) {
...
public:
	...
	inline back_insert_iterator& operator*(void) {
		return (*this);
	}
	inline back_insert_iterator& operator++(void) {
		return (*this);
	}
	inline back_insert_iterator operator++(int) {
		return (*this);
	}
protected:
	Container& m_rCtr;
...
public:
	...
	index_iterate(RandomAccessIterator ibase, IndexIterator iindex) :
		m_Base(ibase), m_i(iindex) {
	}
	inline bool operator==(const index_iterate& i) const {
		return (m_i == i.m_i);
	}
	inline bool operator<(const index_iterate& i) const {
		return (m_i < i.m_i);
	}
	inline bool operator==(const RandomAccessIterator& i) const {
		return (m_Base == i);
	}
	inline bool operator<(const RandomAccessIterator& i) const {
		return (m_Base < i);
	}
	inline IndexIterator base(void) const {
		return (m_i);
	}
	inline reference operator*(void) const {
		return (advance(m_Base, *m_i));
	}
	inline pointer operator->(void) const {
		return (&(operator*()));
	}
	inline index_iterate& operator++(void) {
		++m_i;
		return (*this);
	}
	inline index_iterate& operator--(void) {
	...
	inline index_iterate operator++(int) {
		index_iterate prev(*this);
		++m_i;
		return (prev);
	}
	inline index_iterate operator--(int) {
	...
	inline index_iterate& operator+=(size_t n) {
		m_i += n;
		return (*this);
	}
	inline index_iterate& operator-=(size_t n) {
	...
	inline index_iterate operator+(size_t n) const {
		return (index_iterate(m_Base, m_i + n));
	}
	inline index_iterate operator-(size_t n) const {
		return (index_iterate(m_Base, m_i - n));
	}
	...
	inline difference_type operator-(const index_iterate& i) const {
		return (distance(m_i, i.m_i));
	}
private:
...
template<typename RandomAccessIterator, typename IndexIterator>
inline index_iterate<RandomAccessIterator, IndexIterator> index_iterator(
		RandomAccessIterator ibase, IndexIterator iindex) {
...
inline void indexv_to_iteratorv(typename IteratorContainer::value_type ibase,
		const IndexContainer& xc, IteratorContainer& ic) {
	ic.resize(xc.size());
...
inline typename Container::iterator unconst(
		typename Container::const_iterator i, Container&) {
