...
public:
	...
	struct CPU : public esc::SListItem {
		explicit CPU(uint8_t id,bool bootstrap,uint8_t ready)
			: esc::SListItem(), id(id), bootstrap(bootstrap), ready(ready), curCycles(), lastCycles(),
	...
	typedef esc::SList<CPU>::iterator iterator;
