...
public:
	...
	inline virtual streamsize overflow(streamsize = 1) {
		return (remaining());
	}
	virtual void unlink(void) throw ();
	inline void link(void* p, streamsize n) {
		memlink::link(p, n);
	}
	inline void link(memlink& l) {
		memlink::link(l.data(), l.writable_size());
	}
	inline void link(void* f, void* l) {
		memlink::link(f, l);
	}
	inline void relink(void* p, streamsize n) {
		memlink::relink(p, n);
		m_Pos = 0;
	}
	inline void relink(memlink& l) {
		relink(l.data(), l.writable_size());
	}
...
protected:
	inline void SetPos(uoff_t newPos) {
		m_Pos = newPos;
	}
private:
...
public:
	...
	inline ostream_iterator& operator*(void) {
		return (*this);
	}
	inline ostream_iterator& operator++(void) {
		return (*this);
	}
	inline ostream_iterator operator++(int) {
		return (*this);
	}
	inline ostream_iterator& operator+=(streamsize n) {
		m_Os.skip(n);
		return (*this);
	}
...
inline ostream::ostream(const memlink& source) :
	memlink(source), m_Pos(0) {
}

/// Checks that \p n bytes are available in the stream, or else throws.
inline bool ostream::verify_remaining(const char* op, const char* type,
		size_t n) {
...
template<typename T> struct object_writer {
	inline void operator()(ostream& os, const T& v) const {
		v.write(os);
	}
};
