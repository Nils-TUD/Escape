...
private:
	...
	static const size_t s_nWords = Size / s_WordBits + ((Size % s_WordBits)
			!= 0);
	static const size_t s_nBits = s_nWords * s_WordBits;
private:
	inline value_type& BitRef(uoff_t n) {
		assert (n < Size);
		return (m_Bits[n / s_WordBits]);
	}
	inline value_type BitRef(uoff_t n) const {
		assert (n < Size);
		return (m_Bits[n / s_WordBits]);
	}
...
public:
	inline bitset(value_type v = 0) {
		fill_n(m_Bits, s_nWords, 0);
		m_Bits[0] = v;
	}
	inline bitset(const string& buf) {
		convert_from_bitstring(buf, m_Bits, s_nWords);
	}
	inline void flip(uoff_t n) {
		BitRef(n) ^= Mask(n);
	}
	...
	inline bool test(uoff_t n) const {
		return (BitRef(n) & Mask(n));
	}
	inline bool operator[](uoff_t n) const {
		return (test(n));
	}
	...
	inline value_type at(uoff_t n) const {
		return (test(n));
	}
	...
	inline value_type at(uoff_t first, uoff_t last) const {
		assert (size_t (distance (first, last)) <= s_WordBits && "Bit ranges must be 32 bits or smaller");
		assert (first / s_WordBits == last / s_WordBits && "Bit ranges can not cross dword (4 byte) boundary");
		return ((BitRef(first) >> (first % s_WordBits))
				& BitMask(value_type,distance(first, last)));
	}
	...
	inline bool operator==(const bitset<Size>& v) const {
		return (s_nWords == 1 ? (m_Bits[0] == v.m_Bits[0]) : equal(begin(),
				end(), v.begin()));
	}
	inline const bitset operator&(const bitset<Size>& v) {
		bitset<Size> result;
		transform(begin(), end(), v.begin(), result.begin(), bitwise_and<
				value_type> ());
		return (result);
	}
	inline const bitset operator|(const bitset<Size>& v) {
		bitset<Size> result;
		transform(begin(), end(), v.begin(), result.begin(), bitwise_or<
				value_type> ());
		return (result);
	}
	inline const bitset operator^(const bitset<Size>& v) {
		bitset<Size> result;
		transform(begin(), end(), v.begin(), result.begin(), bitwise_xor<
				value_type> ());
		return (result);
	}
	inline const bitset& operator&=(const bitset<Size>& v) {
		transform(begin(), end(), v.begin(), begin(),
				bitwise_and<value_type> ());
		return (*this);
	}
	inline const bitset& operator|=(const bitset<Size>& v) {
