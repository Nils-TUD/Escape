...
///
/*static*/int string::compare(const_iterator first1, const_iterator last1,
	...
	const size_type len1 = distance(first1, last1), len2 = distance(first2,
			last2);
...
void string::insert(const uoff_t ipo, const wchar_t* first,
		const wchar_t* last, const size_type n) {
	iterator ip(iat(ipo));
...
string::iterator string::insert(iterator start, const_pointer first,
		const_pointer last, size_type n) {
	assert (first <= last);
	...
	start = iterator(memblock::insert(memblock::iterator(start), distance(
			first, last) * n));
...
void string::replace(iterator first, iterator last, const_pointer i1,
		const_pointer i2, size_type n) {
	assert (first <= last);
	...
	if (bti < bte)
		...
		first
				= iterator(memblock::insert(memblock::iterator(first), bti
						- bte));
...
uoff_t string::find(const string& s, uoff_t pos) const {
	...
	for (; i < end() && (i = ::ustl::find(i, end(), endchar)) < end(); i
			+= skip)
...
void string::read(istream& is) {
	...
	if (!is.verify_remaining("read", "ustl::string", szsz))
		return;
	is.read(szbuf + 1, szsz);
	n = *utf8in(szbuf);
	if (!is.verify_remaining("read", "ustl::string", n))
		return;
	resize(n);
	is.read(data(), size());
}
...
void string::write(ostream& os) const {
	...
	*szout = sz;
	...
	if (!os.verify_remaining("write", "ustl::string", szsz + sz))
		return;
	os.write(szbuf, szsz);
	os.write(cdata(), sz);
}
