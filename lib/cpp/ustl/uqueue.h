...
public:
	...
	explicit inline queue(const vector<T>& s) :
		m_Storage(s), m_Front(0) {
	}
	explicit inline queue(const queue& s) :
		m_Storage(s.m_Storage), m_Front(0) {
	}
	...
	inline bool operator==(const queue& s) const {
