...
void vesagui_update(sVESAScreen *scr,void *shmem,gpos_t x,gpos_t y,gsize_t width,gsize_t height) {
	...
	if(rectIntersect(&curRec,&upRec,&intersec)) {
		vesagui_copyRegion(scr,shmem,cursorCopy,intersec.width,intersec.height,
...
static void vesagui_doSetCursor(sVESAScreen *scr,void *shmem,gpos_t x,gpos_t y,int newCursor) {
	...
	if(lastX != x || lastY != y) {
		...
		/* save content */
		vesagui_copyRegion(scr,shmem,cursorCopy,curWidth,curHeight,x,y,0,0,xres,curWidth,yres);
	}
