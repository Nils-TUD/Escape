...
static void vgaInit(void) {
  ...
  while (!installed) /*sleep(1); */usleep(10000);
...
void displayWrite(Word addr, Word data) {
  ...
  if (debug) {
    cPrintf("\n**** DISPLAY WRITE to 0x%08X (x=%d,y=%d), data = 0x%08X ****\n",
            addr, (addr >> 2) & ((1 << 7) - 1), (addr >> 9) & ((1 << 5) - 1), data);
  }
  if (!installed) {
    throwException(EXC_BUS_TIMEOUT);
  }
...
static void keyPressed(unsigned int xKeycode) {
  ...
  if (debugKeycode) {
	  cPrintf("Buffer (%d..%d): ",kbdBufReadPtr,kbdBufWritePtr);
	  for(i = kbdBufReadPtr; i != kbdBufWritePtr; i = KBD_BUF_NEXT(i)) {
		  cPrintf("%02x ",kbdBuf[i]);
	  }
	  cPrintf("\n");
  }
}
