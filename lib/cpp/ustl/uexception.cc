...
bad_alloc::bad_alloc(size_t nBytes) throw () :
	ustl::exception(), m_nBytesRequested(nBytes) {
...
void bad_alloc::info(string& msgbuf, const char* fmt) const throw () {
	if (!fmt)
		fmt = "failed to allocate %d bytes";
	try {
...
libc_exception::libc_exception(const char* operation) throw () :
	exception(), m_Errno(errno), m_Operation(operation) {
	set_format(xfmt_LibcException);
}

/// Copies object \p v.
libc_exception::libc_exception(const libc_exception& v) throw () :
	exception(v), m_Errno(v.m_Errno), m_Operation(v.m_Operation) {
}
...
size_t libc_exception::stream_size(void) const {
	return (exception::stream_size() + stream_size_of(m_Errno)
...
size_t file_exception::stream_size(void) const {
	return (libc_exception::stream_size() + Align(stream_size_of(string(
			m_Filename)), 8));
}
...
/// Initializes the empty object. \p operation is the function that returned the error code.
stream_bounds_exception::stream_bounds_exception(const char* operation,
		const char* type, uoff_t offset, size_t expected, size_t remaining) throw () :
	libc_exception(operation), m_TypeName(type), m_Offset(offset), m_Expected(
			expected), m_Remaining(remaining) {
...
void stream_bounds_exception::info(string& msgbuf, const char* fmt) const throw () {
	...
	if (!fmt)
		fmt = "%s stream %s: @0x%X: need %u bytes, have %u";
	try {
		msgbuf.format(fmt, demangle_type_name(VectorBlock(typeName)),
				m_Operation, m_Offset, m_Expected, m_Remaining);
	} catch (...) {
	}
}
...
size_t stream_bounds_exception::stream_size(void) const {
	return (libc_exception::stream_size() + stream_size_of(m_TypeName)
			+ stream_size_of(m_Offset) + stream_size_of(m_Expected)
