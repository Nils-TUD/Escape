...
else version(Escape)
{
    ...
    private
    {
    	extern (C) int startThread(int function(void *arg),void *arg);
...
class Thread
{
    ...
    body
    {
        ...
        synchronized( slock )
        {
            ...
            else version( Escape )
            {
                ...
                if((m_addr = startThread(cast(int function(void *arg))&thread_entryPoint,cast(void*)this)) < 0)
                	throw new ThreadException("Error creating thread");
            }
            version( OSX )
            {
    ...
    final Throwable join( bool rethrow = true )
    {
        ...
        {
        	if(.join(m_addr) != 0)
        		throw new ThreadException("Not supported yet");
        	// thread is dead now
            volatile m_addr = m_addr.init;
        }
        if( m_unhandled )
    ...
    body
    {
        ...
        {
        	if(.sleep(cast(uint)(period / 10_000)) < 0)
        		throw new ThreadException("Unable to sleep for the specified duration");
        }
    }
...
extern (C) void thread_attachThis()
{
	...
	{
	    Thread thisThread = thread_attach_nolock( gettid(), getStackBottom() );
	}
