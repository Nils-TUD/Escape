...
/*static*/int fstream::om_to_flags(openmode m) {
	static const int s_OMFlags[nombits] = { 0, // in
			IO_CREATE, // out
			IO_APPEND, // app
			IO_APPEND, // ate
			0, // binary
	...
	int flags = (m - 1) & IO_ACCESSMODE; // in and out
...
void fstream::open(const char* filename, openmode mode, mode_t perms) {
	/* TODO perms not supported */
	int nfd = ::open(filename, om_to_flags(mode));
	attach(nfd, filename);
}
...
void fstream::close(void) {
	...
	::close(m_fd);
	/* TODO we can't get interrupted here
	while (::close(m_fd)) {
		if (errno != ERR_INTERRUPTED) {
...
off_t fstream::readsome(void* p, off_t n) {
	...
	do {
		brn = ::read(m_fd, p, n);
	} while ((brn < 0) & (errno == ERR_INTERRUPTED));
	if (brn > 0)
		return (brn);
	/* TODO else if ((brn < 0) & (errno != EAGAIN))
	...
	while (btw) {
		...
		if (bwn > 0)
			...
			if (ios_base::set_and_throw(eofbit | failbit))
				throw stream_bounds_exception("write", name(), pos() - bw, n,
						bw);
			break;
		} else if (errno != ERR_INTERRUPTED) {
			/* TODO if (errno != EAGAIN)*/
...
void fstream::sync(void) {
	// TODO we should provide fsync(fd) which flushes just the blocks from this file
	if (::sync() < 0)
		set_and_throw(badbit | failbit, "sync");
}

/// Get the stat structure.
void fstream::stat(sFileInfo& rs) const {
	if (fstat(m_fd, &rs))
...
memlink fstream::mmap(off_t n, off_t offset) {
	void* result = ::mmap(NULL, n, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd,
			offset);
