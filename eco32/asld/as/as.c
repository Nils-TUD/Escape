...
Value parsePrimaryExpression(void) {
  ...
  if (token == TOK_IDENT) {
    ...
    if (s->status == STATUS_DEFINED && s->segment == SEG_ABS) {
      v.con = s->value;
      v.sym = NULL;
    } else {
      v.con = 0;
...
void roundupSegments(void) {
  while (segPtr[SEG_CODE] & 3) {
    fputc(0, codeFile);
    segPtr[SEG_CODE] += 1;
  }
  while (segPtr[SEG_DATA] & 3) {
    fputc(0, dataFile);
    segPtr[SEG_DATA] += 1;
  }
  while (segPtr[SEG_BSS] & 3) {
    segPtr[SEG_BSS] += 1;
  }
}
...
void writeRealHeader(void) {
  ...
  execHeader.csize = segPtr[SEG_CODE];
  execHeader.dsize = segPtr[SEG_DATA];
  execHeader.bsize = segPtr[SEG_BSS];
  execHeader.crsize = crelSize;
  execHeader.drsize = drelSize;
  execHeader.symsize = symtblSize;
  execHeader.strsize = stringSize;
  conv4FromX86ToEco((unsigned char *) &execHeader.magic);
  conv4FromX86ToEco((unsigned char *) &execHeader.csize);
  conv4FromX86ToEco((unsigned char *) &execHeader.dsize);
  conv4FromX86ToEco((unsigned char *) &execHeader.bsize);
  conv4FromX86ToEco((unsigned char *) &execHeader.crsize);
  conv4FromX86ToEco((unsigned char *) &execHeader.drsize);
  conv4FromX86ToEco((unsigned char *) &execHeader.symsize);
  conv4FromX86ToEco((unsigned char *) &execHeader.strsize);
  fwrite(&execHeader, sizeof(ExecHeader), 1, outFile);
}
...
void writeCodeRelocs(void) {
  ...
  while (f != NULL) {
    ...
    if (f->segment == SEG_CODE) {
      relRec.offset = f->offset;
      relRec.method = f->method;
      relRec.value = f->value;
      relRec.base = f->base;
      conv4FromX86ToEco((unsigned char *) &relRec.offset);
      conv4FromX86ToEco((unsigned char *) &relRec.method);
      conv4FromX86ToEco((unsigned char *) &relRec.value);
      conv4FromX86ToEco((unsigned char *) &relRec.base);
...
void writeSymbol(Symbol *s) {
  ...
  conv4FromX86ToEco((unsigned char *) &symRec.name);
  conv4FromX86ToEco((unsigned char *) &symRec.type);
  conv4FromX86ToEco((unsigned char *) &symRec.value);
