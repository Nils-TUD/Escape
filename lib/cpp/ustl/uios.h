...
public:
	...
	/// I/O state bitmasks.
	enum iostate_bits {
		goodbit = 0, badbit = (1 << 0), eofbit = (1 << 1), failbit = (1 << 2),
#ifndef DOXYGEN_SHOULD_SKIP_THIS
		nbadbits = 3, allbadbits = 0x7
#endif
	};
...
public:
	inline ios_base(void) :
		m_State(goodbit), m_Exceptions(allbadbits) {
	}
	inline iostate rdstate(void) const {
		return (m_State);
	}
	...
	inline operator void*(void) const {
		return (reinterpret_cast<void*> (!fail()));
	}
	...
	inline iostate exceptions(void) const {
		return (m_Exceptions);
	}
	inline iostate exceptions(iostate v) {
		return (m_Exceptions = v);
	}
protected:
	inline bool set_and_throw(iostate v) {
		setstate(v);
		return (exceptions() & v);
	}
	void overrun(const char* op, const char* type, uint32_t n, uint32_t p,
			uint32_t rem);
private:
