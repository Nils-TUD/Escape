...
public:
	...
	inline istringstream& str(const string& s) {
		link(s);
		return (*this);
	}
	inline istringstream& get(char& c) {
	...
	inline istringstream& putback(char) {
		ungetc();
		return (*this);
	}
	inline istringstream& unget(void) {
		ungetc();
		return (*this);
	}
	...
	inline istringstream& read(memlink& buf) {
		return (read(buf.begin(), buf.size()));
	}
	inline istringstream& seekg(off_t p, seekdir d = beg) {
		istream::seekg(p, d);
		return (*this);
	}
...
#endif
	memcpy(m_Delimiters, delimiters, min(strlen(delimiters),
			sizeof(m_Delimiters) - 1));
}
...
template<typename T> struct object_text_reader {
	inline void operator()(istringstream& is, T& v) const {
		v.text_read(is);
	}
};
...
inline istringstream& operator>>(istringstream& is, T& v) {
	typedef typename tm::Select<numeric_limits<T>::is_integral,
			integral_text_object_reader<T> , object_text_reader<T> >::Result
			object_reader_t;
...
template<> struct object_text_reader<string> {
	inline void operator()(istringstream& is, string& v) const {
		is.iread(v);
	}
};
