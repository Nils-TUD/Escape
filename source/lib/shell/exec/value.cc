	...
	switch(v->type) {
		...
		case VAL_TYPE_ARRAY: {
			...
			for(i = istart; i < istart + icount; i++)
				val_append(arr,(const sValue*)vec_get(v->v.vec,i));
			return arr;
		}
	}
	/* never reached */
...
static bool val_doCmp(const sValue *v1,const sValue *v2,uchar op) {
	...
	if(t1 == VAL_TYPE_STR && t2 == VAL_TYPE_STR)
		...
		if(v1c != v2c)
			...
			size_t i,c = 0;
			for(i = 0; i < v1c; i++) {
				bool res = val_doCmp((const sValue*)vec_get(v1->v.vec,i),(const sValue*)vec_get(v2->v.vec,i),op);
				if(res && op != CMP_OP_EQ)
					return res;
				if(res && op == CMP_OP_EQ)
					c++;
...
sValue *val_index(sValue *array,const sValue *index) {
	...
	if(i < 0 || i >= (tIntType)array->v.vec->count)
		return NULL;
	return (sValue*)vec_get(array->v.vec,i);
}
...
static char *val_arr2Str(const sValue *v,bool brackets) {
	...
	for(i = 0; i < len; i++) {
		size_t slen;
		sValue *el = (sValue*)vec_get(v->v.vec,i);
